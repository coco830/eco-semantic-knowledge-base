...
## EcoCheck candidate coverage / industry influence factor samples v1.0

本入口承接 EcoCheck 候选覆盖、行业画像中层规则、证据/拍照要求下沉和负样本防误触发的样本沉淀。它是已审核行业影响因子样本包，不是 approved baseline，接入 EcoCheck 行业特性生成器和中层规则引擎，不生成正式检查模板、许可类型、扣分或排污系数。

- `ecocheck_candidate_coverage_industry_influence_factor_samples_v1_0.md`: 已审核行业影响因子样本治理说明。
- `ecocheck_candidate_coverage_industry_influence_factor_samples_v1_0.csv`: 48 条已审核样本，包含 40 条正向行业影响因素样本和 8 条负样本。
- `ecocheck_candidate_coverage_industry_influence_factor_samples_manifest_v1_0.json`: 已审核行业影响因子样本包 manifest，锁定 CSV/doc hash、运行时禁区和非系数边界。
- `ecocheck_candidate_coverage_industry_influence_factor_samples_gate_report_v1_0.json`: validator 输出的门禁报告。
- `validate_ecocheck_candidate_coverage_industry_influence_factor_samples_v1_0.py`: 已审核行业影响因子样本包验证入口。
