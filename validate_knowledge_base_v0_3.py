...
def main():
    ...
    raw_permit = read_csv(artifact_path("permit_management_catalog_table_cells.csv"))
    ...
    permit_conditions = read_csv(artifact_path("all_permit_condition_backfill_v0_3.csv"))
    ...
    context = read_csv(artifact_path("all_context_applicability_review_v0_3.csv"))
    ...
    scenario_gov = read_json(artifact_path("scenario_template_governance_v0_3.json"))
    ...
    manifest = read_json(artifact_path("knowledge_base_manifest_v0_3.json"))
    ...
    (artifact_path("knowledge_base_v0_3_validation_report.json")).write_text(
        json.dumps(report, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    fail_fields = ["file", "reason", "industry_code", "scenario_id", "row", "bad"]
    with (artifact_path("knowledge_base_v0_3_failure_list.csv")).open("w", encoding="utf-8-sig", newline="") as f:
